...
int main(int argc, char *argv[]) {
  int ii, j, k, l, m, n, mm, PART, flag, dumpi, nn, maxmoment, conf, confnum, comp[2][3], rect;
  ...
  int near[PART][28], nearc[PART], bond, nearcomp[PART][3], solid[confnum][PART], interface[PART][500], interfacec[PART], fsolid[PART], phase[3][PART], phasec[3];
  double qlm[PART][2][13], qlbar[PART], qltilde[PART][2][13], value, sum, qldot, rmin;
  for(j=0;j<PART;j++) {
    fgets(line,300,ionfile);
  ...
  for(j=0;j<phasec[0];j++) {
    rmin=BOXSIZE[2];
    for(k=0;k<phasec[2];k++) {
      for(n=0;n<3;n++){
	rp[n]=findshort(pos[3*phase[0][j]+n],pos[3*phase[2][k]+n],&lr[n],BOXSIZE[n]);
	rp[n]*=lr[n];
      }
      r=sqrt(rp[0]*rp[0]+rp[1]*rp[1]+rp[2]*rp[2]);
      if(r<rmin)
    ...
    if(charge[phase[0][j]]<7.0)
      printf("1 %d %lf\n",phase[0][j],rmin);
    else if(charge[phase[0][j]]<9.0)
      printf("2 %d %lf\n",phase[0][j],rmin);
    else
      printf("3 %d %lf\n",phase[0][j],rmin);
  }
  for(j=0;j<phasec[1];j++) {
    rmin=BOXSIZE[2];
    for(k=0;k<phasec[2];k++) {
      for(n=0;n<3;n++){
	rp[n]=findshort(pos[3*phase[1][j]+n],pos[3*phase[2][k]+n],&lr[n],BOXSIZE[n]);
	rp[n]*=lr[n];
      }
      r=sqrt(rp[0]*rp[0]+rp[1]*rp[1]+rp[2]*rp[2]);
      if(r<rmin)
    ...
    if(charge[phase[1][j]]<7.0)
      printf("4 %d %lf\n",phase[1][j],rmin);
    else if(charge[phase[1][j]]<9.0)
      printf("5 %d %lf\n",phase[1][j],rmin);
    else
      printf("6 %d %lf\n",phase[1][j],rmin);
  }
