...
!  File type: x4
!  ------------------
!  Read/write time-stamped, unformatted real*4 configuration file containing
!  positions:
!
!     Record 1:
!     ----------------
...
!       .    .    .
!       .    .    .
!     x(n) y(n) z(n)         particle n coordinates, real*4
!
...
!  File type: x8
!  ------------------
!  Read/write time-stamped, unformatted real*8 configuration file containing
!  positions.
!
!     Record 1:
!     ----------------
...
!     Record 2:
!     ----------------
!     x(1) y(1) z(1)   particle 1 coordinates, real*8
!     x(2) y(2) z(2)   particle 2 coordinates, real*8
!       .    .    .  
!       .    .    .  
!       .    .    .  
!     x(n) y(n) z(n)   particle n coordinates, real*8
!
...
!  File type: md0
!  ------------------
!  Read/write an unformatted real*8 configuration file created by version MD_0
!  of the MD code. This type is almost the same as xv8, except it does not have
!  a timestamp. It consists of just one record,
!
!     Record 1:
!     ----------------
!     x(1) y(1) z(1) vx(1) vy(1) vz(1)   particle 1 position, velocity, real*8
...
!     x(n) y(n) z(n) vx(n) vy(n) vz(n)   particle n position, velocity, real*8
!
!  The calling program sends read_md0 the number of particles n that it wants
!  from the file. The first n will be read and returned. If n is set to 0, all
!  particles will be returned, and n will be set to the number of particles.
!  When writing, the program must send n to write_xv8, so it will know how many
!  particles to write.

      subroutine read_md0(file,n,x8,v8)
      implicit none
      character*(*)  file     !file to read
      integer        n        !number of particles
      real*8         x8(3,n)  !positions
      ...
      if(n.eq.0) then
        call file_stat(trim(file),size)
        n=size-(4+4)    !record 2: (4+4 bytes for record markers)
        n=n/(6*8)       !record 2: 3 real*8 coordinates and velocities
      endif
      open(UNIT=13, FILE=trim(file), STATUS='OLD', FORM='UNFORMATTED')
      read(13) (x8(1,i),x8(2,i),x8(3,i),v8(1,i),v8(2,i),v8(3,i), i=1,n)
      close(13)
      return
      end subroutine read_md0

      subroutine write_md0(file,n,x8,v8)
      implicit none
      character*(*)  file     !file to write
      integer        n        !number of particles
      real*8         x8(3,n)  !positions
      ...
      close(14)
      return
      end subroutine write_md0


!*******************************************************************************
!  File type: xvf
!  ------------------
!  Read/write time-stamped, formatted configuration file containing n+1 records.
!
!     Record 1:    timestamp
!     Record 2:    x(1) y(1) z(1) vx(1) vy(1) vz(1)
!     Record 3:    x(2) y(2) z(2) vx(2) vy(2) vz(2)
!        .           .    .    .     .     .     .
!        .           .    .    .     .     .     .
!        .           .    .    .     .     .     .
!     Record n+1:  x(n) y(n) z(n) vx(n) vy(n) vz(n)
!
!  The calling program should send the number of particles n in the file. But
!  if this is unknown, set n=0, and the reader should be able to figure it out.
!  It will simply read until reaching an end-of-file, and set n to the number
!  of particles read. This number is returned to the calling program.
...
!  File type: md1
!  ------------------
!  Read/write formatted configuration file containing n records created by 
!  version MD_1 of the MD code. This is almost the same as type xvf, except it
!  contains no timestamp record.
!
!     Record 1:  x(1) y(1) z(1) vx(1) vy(1) vz(1)
!     Record 2:  x(2) y(2) z(2) vx(2) vy(2) vz(2)
!        .         .    .    .     .     .     .
!        .         .    .    .     .     .     .
!        .         .    .    .     .     .     .
!     Record n:  x(n) y(n) z(n) vx(n) vy(n) vz(n)
!
!  The calling program should send the number of particles n in the file. But
!  if this is unknown, set n=0, and the reader should be able to figure it out.
!  It will simply read until reaching an end-of-file, and set n to the number
!  of particles read. This number is returned to the calling program.
      ...
      subroutine read_md1(file,n,x8,v8)
      implicit none
      character*(*)  file     !file to read
      integer        n        !number of particles
      real*8         x8(3,n)  !positions
  ...
  200 continue
      ...
      close(13)
      if(n==0) n=i
      end subroutine read_md1

      subroutine write_md1(file,n,x8,v8)
      implicit none
      character*(*)  file     !file to write  (in)
      integer        n        !number of particles  (in)
      real*8         x8(3,n)  !positions  (in)
...
!
!     Record 1:    timestamp
!     Record 2:    x(1) y(1) z(1) vx(1) vy(1) vz(1) Z(1) A(1)
!     Record 3:    x(2) y(2) z(2) vx(2) vy(2) vz(2) Z(2) A(2)
!        .           .    .    .     .     .     .    .    .
!        .           .    .    .     .     .     .    .    .
!        .           .    .    .     .     .     .    .    .
!     Record n+1:  x(n) y(n) z(n) vx(n) vy(n) vz(n) Z(n) A(n)
!
!  Z is the charge (atomic number), and A is the mass (atomic weight). The calling
