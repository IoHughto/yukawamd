...
int main(int argc, char *argv[]) {
  ...
  double rho, xl[3], dump, relpos[3], r, bind, time, ev, ek, px, pp[9], BOXSIZE;
  char line[300], filename[100], jname[3], kname[3], dumpc[100];
  FILE *out, *ionfile, *in;
  float dumpf;
  ...
  for(iter=0;iter<confnum;iter++) { 
    ...
    if(rank==0) { 
      sprintf(filename,"md.%011ld.xv8b",startt+dt*iter); 
      printf("%s\n",filename); 
      in=fopen(filename,"r");
    ...
    ierr=MPI_Bcast(xl,3,MPI_DOUBLE,0,MPI_COMM_WORLD); 
	...
	for(n=0;n<3;n++) { 
   	  relpos[n]=findshort(pos[3*j+n],pos[3*k+n],&dump,xl[n]); 
   	} 
