...
int main(int argc, char *argv[]) {
  ...
  int PART, binnum, n, j, k, l, m, iter, flag, ionnum[5], dt, dumpi, ierr, rank, count, compnum, conf, slices, sflag[3];
  ...
  FILE *xyz[slices][slices][slices];
  for(j=0;j<slices;j++) {
    for(k=0;k<slices;k++) {
      for(l=0;l<slices;l++) {
	sprintf(filename,"nn.%d.%d.%d.xyz",j+1,k+1,l+1);
	xyz[j][k][l]=fopen(filename,"w");
      }
    }
  }
  ...
  for(conf=0;conf<confnum;conf++) {
	...
	if(k!=j) {
      ...
      for(m=0;m<slices;m++) 
	if(sflag[0]==slices+1) 
	  if(pos[3*j]<sliced*(m+1))
	    sflag[0]=m;
      sflag[1]=slices+1;
      for(m=0;m<slices;m++) 
	if(sflag[1]==slices+1) 
	  if(pos[3*j+1]<sliced*(m+1)) 
	    sflag[1]=m;
      sflag[2]=slices+1;
      for(m=0;m<slices;m++) 
	if(sflag[2]==slices+1) 
	  if(pos[3*j+2]<sliced*(m+1)) 
	    sflag[2]=m;
      for(k=0;k<14;k++) {
	fprintf(xyz[sflag[0]][sflag[1]][sflag[2]],"C %e %e %e\n",x[k],y[k],z[k]);
	for(l=0;l<14;l++) {
	  if(k!=l) {
