...
{
  MPI_Status status;
  int j, n, k, PART, a[3], b, c, confnum, conf, ierr, rank, count, compnum;
  float time4;
  double BOXSIZE[3], epsilon, PE[6][5], recv[6][5], potential, r, relpos[5][3], CHARGE, SCRLEN, CUTOFF, u[3][5][3], v[3][5][3], ahc, first, second, rp[3], rho, time8;
  ...
  for(conf=0;conf<confnum;conf++) {
    ...
    ierr=MPI_Bcast(pos[0],3*PART,MPI_FLOAT,0,MPI_COMM_WORLD);
    for(b=0;b<6;b++)
      for(c=0;c<5;c++){
	PE[b][c]=0.0;
	recv[b][c]=0.0;
    ...
    for(j=rank*compnum;j<(rank+1)*compnum;j++) {
      /* if((j-rank*compnum+1)%32==0) */
      /* 	printf("Process %d is %d/8 done.\n",rank,(j-rank*compnum+1)/32); */
      for(k=0;k<j;k++) {
	if(k!=j) {
	  for(a[0]=0;a[0]<3;a[0]++) {
	    for(a[1]=0;a[1]<3;a[1]++) {
    ...
    ierr=MPI_Allreduce(PE,recv,30,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
    ...
    if(rank==0) {
      ...
      /* 	recv[5][b]/=PART; */
      /* } */
      printf("%4.3f %16.14e ",epsilon,recv[0][0]);
      for(b=0;b<6;b++) {
	first=(2.0*(recv[b][1]-recv[b][2])/3.0-(recv[b][3]-recv[b][4])/12.0)/epsilon;
	second=(4.0*(recv[b][1]+recv[b][2])/3.0-(recv[b][3]+recv[b][4])/12.0-5.0*recv[b][0]/2.0)/(epsilon*epsilon);
