...
int main(int argc, char *argv[]) {
  ...
  float vel[3*PART], skip[PART][3];
  float recvbuf[confmax][3*compnum];
  ...
  for(conf=0;conf<confnum;conf++) {
    ...
    if(load[conf]==0) {
      if(rank == 0) 
	read_xv4a_(filename,&PART,&time4,&skip[0][0],&vel[0],k);
      load[conf]=1;
      ierr=MPI_Scatter(vel,3*compnum,MPI_FLOAT,recvbuf[conf%confmax],3*compnum,MPI_FLOAT,0,MPI_COMM_WORLD);
    }
	...
	if(rank == 0) 
	  read_xv4a_(filename,&PART,&time4,&skip[0][0],&vel[0],k);
	load[other]=1;
	ierr=MPI_Scatter(vel,3*compnum,MPI_FLOAT,recvbuf[other%confmax],3*compnum,MPI_FLOAT,0,MPI_COMM_WORLD);
      }
      for(j=0;j<compnum;j++) {
