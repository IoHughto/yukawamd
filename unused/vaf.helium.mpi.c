...
int main(int argc, char *argv[]) {
  ...
  sprintf(filename,"%s.He.out",argv[3]);
  Heout=fopen(filename,"w");

  float vel[confmax][3*PART], skip[PART][3];
  ...
  for(conf=0;conf<confnum;conf++) {
    ...
    if(load[conf]==0) {
      if(rank == 0) 
	read_xv4a_(filename,&PART,&time4,&skip[0][0],&vel[conf%confmax][0],k);
      load[conf]=1;
      ierr=MPI_Bcast(vel[conf%confmax],3*PART,MPI_FLOAT,0,MPI_COMM_WORLD);
    }
    end=min(conf+confmax,confnum);
    for(other=conf;other<end;other++) {
      if(load[other]==0) {
	if(rank == 0) 
	  read_xv4a_(filename,&PART,&time4,&skip[0][0],&vel[other%confmax][0],k);
	load[other]=1;
	ierr=MPI_Bcast(vel[other%confmax],3*PART,MPI_FLOAT,0,MPI_COMM_WORLD);
      }
      for(j=rank*compnum;j<(rank+1)*compnum;j++) {
	pass=vel[conf%confmax][3*j]*vel[other%confmax][3*j]+vel[conf%confmax][3*j+1]*vel[other%confmax][3*j+1]+vel[conf%confmax][3*j+2]*vel[other%confmax][3*j+2];
	if(charge[j]<5.0)
